...
class Kiwoom(QMainWindow, form_class):
    ...
    def receive_real_data(self, code, real_type, real_data): 
        ...
        if self.possible_time == 1 :
            ...
            try :
                if code not in self.item_slot :
                    price_cur = int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", code, 10).replace('+', '').replace('-', '').strip())       ## 매도호가 수량 1
                    price_start = int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", code, 16).replace('+', '').replace('-', '').strip())       ## 매도호가 수량 1
                    price_high = int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", code, 17).replace('+', '').replace('-', '').strip())       ## 매도호가 수량 1
                    price_low = int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", code, 18).replace('+', '').replace('-', '').strip())       ## 매도호가 수량 1

                    ratio_max_low = round((((price_low - price_start) / price_start) * 100), 2)
                    ratio_cur = round((((price_cur - price_start) / price_start) * 100), 2)
                    print(code, "max low : ", ratio_max_low, ratio_cur)

                    if ratio_cur <= -3 :
                        try:
                            item_code = code
                            price = int(self.kiwoom.dynamicCall("GetCommRealData(QString, int)", code, 27).replace('+', '').replace('-', '').strip())       ## 최우선 매도호가
                            qty = math.floor(1000000 / price)

                            # already = self.which_thread(item_code)[0]
                            th_num = self.which_thread(item_code)[1]
                            
                            print("[MAIN RCV REAL ORDER_BUY] : ", item_code, qty, price, th_num)
            ...
            if slot != -1 :
                try :
                    ...
                    print("price2 : ", price_sell, price_buy)
    ...
    def check_real(self, data) :
        ...
        for i in range(len(self.item_slot)) :
            if self.item_slot[i] != 0 :
                self.SetRealReg("0101", self.item_slot[i], "10;41", "1")      ## 실시간 데이터 수신 등록
