...
class Kiwoom(QMainWindow, form_class):
    ...
    def btn_test(self) :
        ...
        # print("current : ", self.test)

        # code1 = "005930"
        # code2 = "015760"
        # if self.test == 0 :
        #     # self.SetRealReg(1001, code, 10, 1)
        #     self.SetRealReg(1001, code1, 10, 0)
        #     self.SetRealReg(1001, code2, 10, 1)
        #     self.test = 1
        # elif self.test == 1:
        #     # self.SetRealRemove(1001, code)
        #     self.SetRealRemove(1001, code1)
        #     self.SetRealRemove(1001, code2)
        #     self.test = 0
        # # self.kiwoom.dynamicCall("SetRealReg(QString, QString, QString, QString)", "0101", code, 10, 0)
    ...
    def func_SET_Items(self, rqname, trcode, recordname):
        ...
        for i in range(int(data_cnt)):
            ...
            self.func_SET_TableData(1, i, 0, item_code.replace('A','').strip(), 0)
        ...
        self.func_SET_RTData(data_cnt)

    def func_SET_RTData(self, data_cnt):
        self.item_codes = []
        for i in range(data_cnt):
    ...
    def SetRealReg(self, scrrenNo, item_code, fid, realtype):
        self.kiwoom.dynamicCall("SetRealReg(QString, QString, QString, QString)", "0101", item_code, "10", realtype)
    def SetRealRemove(self, scrrenNo, item_code):
        self.kiwoom.dynamicCall("SetRealRemove(QString, QString)", "0101", item_code)
    ...
    def receive_real_data(self, code, real_type, real_data): 
        ...
        for i in range(len(self.item_codes)) :
            if code == self.item_codes[i]:
                ...
                price_buy = price_buy.replace('+', '').strip()
                self.func_SET_TableData(1, i, 5, price_buy, 0)
                self.func_SET_TableData(1, i, 6, price_sell.replace('+', '').strip(), 0)
                ...
                self.func_SET_TableData(1, i, 11, str(total_sum), 0)

                percent = round((total_sum / total_purchase), 1)
                # percent = total_sum / total_purchase
                print("Per : ", percent)
                if percent > 0:
