...
def run():
    ...
    df_last = pd.DataFrame(columns = ['code', 'ratio', 'ratio_price'])
    ...
    # for i in range(10) :
        ...
        try :
            ...
            df2 = df[['종가']]
            recent_price = int(df2.iloc[0]['종가'])
            ratio_price = round((recent_price / mean_price), 2)

            if ratio_price >= 0.95 and ratio_price <= 0.99 :
                min_v = float(df2.min())
                max_v = float(df2.max())
                ratio = round((max_v / min_v), 2)

                df_last.loc[len(df_last)] = [code, ratio, ratio_price]
